...
class DistribusiController extends Controller
{
    ...
    public function riwayat(Request $request)
    {
        ...
        $summary = [
            ...
            'diterima' => $distribusis->whereIn('status', ['Diterima', 'Diterima Sebagian'])->count(),
            'di_perjalanan' => $distribusis->whereIn('status', ['Di Perjalanan', 'Terkirim', 'Dikirim'])->count(),
            'kendala' => $distribusis->where('status', 'Kendala')->count(),
            'pending' => $distribusis->where('status', 'Pending')->count(),
        ];

        return view('distribusi.riwayat', compact('distribusis', 'tanggal', 'summary'));
    }
